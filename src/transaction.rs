...
impl Transaction {
    ...
    pub fn from_csv_line(line: &str) -> Result<Transaction, String>{
        ...
        // fields = {"3","101","Amazon","2023-02-20",....}

        // b. check if the length of fields is equals to 7, 
        // if it is not, return an Err with an appropriate message in String
        if fields.len() != 7{
            return Err("Incorrect number of fields parsed from the CSV file".to_owned());
        } 
        ...
        // let client_id = fields[1].parse::<u32>().expect("Parsing of transaction ID was unsuccessful");
        ...
        //Create a type Transaction and return it based on the temporary values parsed & prepared from above.
        Ok(transaction)
    }
}
